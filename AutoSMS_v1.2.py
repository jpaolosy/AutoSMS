...
def readMail():
    ...
    try:
        ...
        imap_obj.select_folder('INBOX', readonly=False) # READ ONLY TOGGLE
...
def updateCounter(counter_object):
    ...
    with open ("/home/rctdm/Desktop/AutoSMS/json/counterLog.json") as counter_append:
        json_data = json.load(counter_append)
    if counter_object != "credits_available":
        json_data["counterLog"][counter_object] += 1
    else:
        json_data["counterLog"][counter_object] -= 1
    with open("/home/rctdm/Desktop/AutoSMS/json/counterLog.json", "w") as counter_save:
        counter_save.write(json.dumps(json_data))
        
		
def printCounterValues():

    with open ("/home/rctdm/Desktop/AutoSMS/json/counterLog.json") as counter_values:
